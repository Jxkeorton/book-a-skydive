...
class AFFCourse(models.Model):
    ...
    def clean(self):
        ...
        if AFFCourse.objects.filter(date__week=self.date.isocalendar()[1]).exclude(id=self.id).exists():
...
class VisitorDetail(models.Model):
    course = models.ForeignKey(AFFCourse, related_name='bookings', on_delete=models.CASCADE)
